function Expand-AkamaiError {
    ...
    # Load known errors
    $KnownErrors = Get-Content -Raw $PSScriptRoot/KnownErrors.json | ConvertFrom-Json

    # Extract response content type
    $ResponseContentType = ''
    if ($PSVersionTable.PSVersion.Major -ge 6) {
        $ResponseContentType = $ErrorRecord.Exception.Response.Content.Headers.ContentType.MediaType
    }
    else {
        $ResponseContentType = $ErrorRecord.Exception.Response.ContentType
    }

    # If json, convert to object to extract useful info
    $ErrorData = $null
    if ($ResponseContentType -and $ResponseContentType.Contains('json') -and $ErrorRecord.ErrorDetails.Message -match '^[\s]*\{[\s\S]*\}[\s]*$') {
        try {
            ...
            'title', 'errors', 'detail' | ForEach-Object {
                if ($ErrorData.$_ -and $ErrorData.$_.EndsWith('.')) {
    ...
    elseif ($null -ne $ErrorData.status -and $ErrorData.errors -is 'String') {
        Write-Debug "Setting errormessage from title and errors."
    ...
    if ($null -ne $ErrorMessage) {
        ...
        $KnownError = $KnownErrors | Where-Object { $ErrorData.Detail -like "*$($_.Error)*" }
        if ($KnownError) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCuL+8cg5ZEGDOG
# D8ZZuTBjvimddCEMLDbRlH4A4jco9KCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgJbs9mGO5fD1K/QrKxLZUqgiwIBBwIXjkrs6zsbOeVnsw
# DQYJKoZIhvcNAQEBBQAEggIAlFyPKRn1EjaFKQB3JaQ1gkERXi+yxtVeT2IsN2Ue
# N1qCYOTPI20//AHoMW+vetBjRcpZsstifZK+BffrPCJ4zaoMvB6pWhi3iQsdT0qw
# IatoYptM9w1EoI//z1yguTGAHBVyoSNGsbCzZCLuuwK74ft3RM4LZrZp9icuWHj7
# gWcWBGJLZcEod3mB2rRHT+1XptOeXXhdUgSxY+bmakGnMZp52OlEWkS3sJ9tV/IV
# rO2f2V6RVCVv1pTALYJ3/rKMtytHZt4upW3gIBd0suk5vmXffqD5PJcg3G0rRiEp
# C9wYV8jWJJePj0RrhhC9FDOogN7YVglZ0sTWIvqqGvk3vUFvGKhnlu9gVG4XsFMm
# CHVsfoq0f+blMspT5mBTJPbjgh7MrLNttAJ99ghn9E1QRPy2ipVz+9TtvXYk5wO9
# 6yQ8rCDdqw22uJ6WA2UJWH4wjOlm4l4mIw2bp/RyyWuOSzkhAuz4QNbGMvDauuRx
# UsBsjQ3/IoLJxseLbhrwKUJlIyObfL6aFmIWldhgcN+r4uFR9oMLTI4r7YWUFL6B
# BJg3ipZHYGo3wfryMPv7oD4AGw9BWavprTlUkgynxd4SrXI9cCt1S/y65gfTYYs0
# HglDUykARVB+EVH0CfFMYJvW+3wufCWRl1kCpiRXBeZyBLuMIaIRGnkSNKGClOMF
# bXyhghc5MIIXNQYKKwYBBAGCNwMDATGCFyUwghchBgkqhkiG9w0BBwKgghcSMIIX
# DgIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb