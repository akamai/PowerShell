<#
  ...
  .DESCRIPTION
  The `New-AkamaiSession` cmdlet creates a new Akamai Session variable. Used by the `Invoke-AkamaiRestMethod` cmdlet's authentication scheme.

  .Parameter ClientSecret
  An API client's client secret.

  .Parameter Host
  An API client's host address. Combined with a method's path, it creates a fully-formed HATEOAS entry point URL.

  .Parameter AccessToken
  An API client's access token.
  ...
  Alias = Section. The environment for which the authorization information is used. Default is `default`.

  .Parameter EdgeRCJSON
  The path to a JSON file that contains API credentials. Multiple credential sections allowed. If used, individual authorization parameters are ignored. 
...
Function New-AkamaiSession {
    ...
    if ($EdgeRCJSON) {
        ...
        foreach ($p in $AuthInfo.PSObject.Properties) {
            $AuthSection = $p.Value
            if ($null -eq $AuthSection.client_token -or $null -eq $AuthSection.access_token -or $null -eq $AuthSection.client_secret -or $null -eq $AuthSection.host) {
        ...
        if ($AccountSwitchKey) {
            $NewAuth | Add-Member -NotePropertyName 'account_key' -NotePropertyValue $AccountSwitchKey
        }
        if ($Passthrough.IsPresent) {
            ...
            if ($null -ne $Script:AkamaiSession.Auth) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCADWbmt62Uv2FPJ
# E/gtSe/OsgUtom+tThDOdidPXxThe6CCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgD6HYCemg3SmisFk/sjaleqcYn+bv1Fy2C54HB3+9g5cw
# DQYJKoZIhvcNAQEBBQAEggIAo1KPWeuJOhk5fjIlqWNMAE+fR/Khp1rgEcPqBXqb
# 8hizQVTLogCpp8ubilSL42EQkt+s3QrcQuynyUVpbblkzcQT/IU4BpD6dpfzn/jf
# a3Q0VSPPv2oWPTyg+xYHZc8ljc7EIAQR8UF4kmbfNyl1nAsD8Kb19YCq56Eq8vOl
# 3KQkAAbungCO0gLKoS3iiPeEXWmQInYpEdu3MTBvR5RLwnyBiEjZhB3AaFcga8vh
# IzJKjjGtv2nVFwkzba9So447j+XFY11S/vWwAzhjiWykCGKJqcjaGmS+8Us666hW
# ZNQepMdSe++q2AdEaKzDYz/f6ugyeE9ZAQTgoXCuPApzPyE5D5QtJFJU9VMqIJ//
# 2nG+W0KKfmhblx1iFn9Dr64Cg4M65gKb2+3fhL9r/nBirNCb6UZkHxyeT6DM0gzr
# 2BzqnbvBaJ8+zOv7gISJtVFcjxyYcCD3wYphm1VC88OEP0J6UuQPyUBJ3vcx6eeH
