...
function Get-METSCASet {
    ...
    process {
        ...
        if ($CASetID) {
            $Path = "/mtls-edge-truststore/v1/ca-sets/$CASetID"
            $AcceptHeader = 'application/vnd.akamai.mtls-edge-truststore.ca-set.v1+json'
        }
        else {
            $Path = "/mtls-edge-truststore/v1/ca-sets"
            $AcceptHeader = 'application/vnd.akamai.mtls-edge-truststore.ca-sets.v1+json'
        }
        $AdditionalHeaders = @{
            'accept' = $AcceptHeader
        }
        ...
        if ($Response.Status -lt 300 -and $AkamaiOptions.EnableDataCache) {
            if ($CASetID) {
                ...
                $DeletedCASets = @($Response.Body.caSets | Where-Object isDeleted)
                $LiveCASets = @($Response.Body.caSets | Where-Object isDeleted -eq $false)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA+QPwJNpu79RJZ
# sLScI1a5YZdWtSbz4abau0ea0S/jFKCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgKusbLDOs+Q931bQcf8O0jUq2JXNlD2NfI9wbHTI/G30w
# DQYJKoZIhvcNAQEBBQAEggIAkBEjycVXm2g8TDAYsVHO428GWCD7K+keFyzNUpgX
# 2Yv5bMxZmAtyB8EitxUEYmOx4e479FLaWKszJB7TkvkpcUaX39hLI3vyedUTh5R3
# VWHnA0MmaerAgUB4BUO1b8Lcg4wrYe7GmpcoUl25fwwECadNx+1Tm34T7UkwmvpL
# ehomSDlQZdTJr/+a33V9A48Xm26ooW9srEsS/kf7pPhoTmUgKJjFFpb67vq7Mccg
# aBPB1OD86koFOqjUz4PIzz0uC8kF59idNjayKWEtSIvEaK47ukZCMJV/EZNXaApH
# QGL4sVp5AOYGt5Z/s5s/IbCmeGNAQt6RdzP9UpyHAUlidv8UIh1m1rjfAP/VQSAn
# b8Z0H/ObVvFI27umXl3pPPoUPvE5o6yfN5f7PYt7Wg+hjb0TOO+xjUXM6iqQ32/o
# 6ofkeztqnOHxEOaYVrHibVkJNkMmBRGnNsrFchF4vmH8DZbL2/whLcAeLUtXP0nc
# phmsvd7fZvGa801Rtxkeo+9/xtcdkSBsuBA9wJ/ZUSayEXIBzGdOejEO+EfcCIzu
# iVxjmsv9NKopIwwx8K/DWyle0OCIWev1dHnu0qIPpugesuRt9pmDeKE/ZVaqmh/V
# QgKrruuKzQirgmUSqFbLNg6KLQwnAx7GfT0MsGLAhE15mvQNn4fHjDXVgkxMRRkw
# 1MShghd3MIIXcwYKKwYBBAGCNwMDATGCF2MwghdfBgkqhkiG9w0BBwKgghdQMIIX
# TAIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEEoGkEZzBlAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgsXAoM+9IcX6COu87l/pnxcECiFHl
# RS2e6wsYssCw3IwCEQDa6jcyK/CSbSW2zD0QR58lGA8yMDI1MDgwNTE4MDIwMFqg
...
# MC8GCSqGSIb3DQEJBDEiBCCDHnRtC+cbx3ANC9nyfcWk5aCYUlAejfFsS/82NyBi
# 2TA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCBKoD+iLNdchMVck4+CjmdrnK7Ksz/j
# bSaaozTxRhEKMzANBgkqhkiG9w0BAQEFAASCAgABEJIE3/XnXKu9MjPiVTPVAS6m
# QKRXFDmzT17jAkmbSM0zlGxtT0WUInodhtrLCtCUV7XWZ95l7IkZDHPFZupm+rV5
# giWNBsyYz66bk0/4m8N8MwFcgUVLJyzIoieGgKLX9wRQ/hsoABTVvh+MTG/BpkD2
# 2/QmkEQUEZ9iU+d0ehtrZtBK3L1wFJnZjE2SxGZH61oFJxdmjLBcurLatxHwUmh0
# ELWHiuyjmweEt85OaFG4Fd9fO8g2wq9VGTBJzcYvXQTLUNGtzsbXLzj/ebSMxdym
# Bh1ZmvNHpxPLIjKdDlaS8iojm3pEtDLse357GE7TA4smw71Gct5MgFI8OW5tG9aK
# udpWWbKbC+m60oGDtoxLIpkbXcYjOs+N5c3M7JRPrP+bbaeZ36/UBltVASMX/L0l
# aqt3OdcBIeiYzCILoIjYVa0ISjDYtTJUCNW5W3QM2Ag24WlJ0MU5rVjYuknrCeeA
# MmD4lH4unoBy9NEHwaGMONWZqkwTsES0uqUFD/i0exXYtRU3sqtuHmMZSpC0qQHu
# FSOozN8sfuALCEilaZY7OnyPIF4iw8WoNb/mC56GvCPlMJ4fyQRxD6W9xvPyJ6ZC
# 6fEKwP5DqbEfQRsWa2b5uvtzo6LIY0j1rSRw8/a9QpB/d6zGcBoOXC/1pkP0ClhS
# SgdpESIbQYdWplyREg==
