...
function Get-METSCASetActivation {
    [CmdletBinding(DefaultParameterSetName = 'name-all')]
    Param(
        [Parameter(ParameterSetName = 'name-all')]
        [Parameter(ParameterSetName = 'name-single', Mandatory)]
        [string]
        $CASetName,

        [Parameter(ParameterSetName = 'id-all')]
        [Parameter(ParameterSetName = 'id-single', Mandatory)]
        [int]
        $CASetID,
    ...
    process {
        ...
        if ($ActivationID) {
            $Path = "/mtls-edge-truststore/v1/ca-sets/$CASetID/versions/$VersionID/activations/$ActivationID"
            $AcceptHeader = 'application/vnd.akamai.mtls-edge-truststore.activation.v1+json'
        }
        else {
            $Path = "/mtls-edge-truststore/v1/activations"
            $QueryParameters = @{ 
                'caSetId'   = $CASetID
                'versionId' = $PSBoundParameters.VersionID
            }
            $AcceptHeader = 'application/vnd.akamai.mtls-edge-truststore.activations.v1+json'
        }
        $AdditionalHeaders = @{
            'accept' = $AcceptHeader
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDJRRRzL8qb//qi
# 9K/OezIE62ckQ2JJ6hLU44tMuZws1aCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgeuryAQ2twCGE+Hi4V/Oby5UOk8vVHfcYPnUSguzj3fww
# DQYJKoZIhvcNAQEBBQAEggIAmXWylc6MfXW2OusEkPS55syoDD2vW0pwK7jfEYZu
# X41IowTMfOiYDQYnW74cqX1uYQlIuFKQM8Vgrmokj4kMnIIaT7dPmLMfUCwYe5p+
# 3Q6mMWKBPftDAHGlJiaYz3crKgj8JXOdGI2c74gdQcRjHrdNVmgkdi6U2vOzMybd
# IUrcWf1/eGAJO+Gl23GiHcG+LWIpUR/dpPA3Uk5infmg7h+necgikSu21MxPI9qN
# de9AzVn8erjElImzr6f5AaZGpLem0/zta2kDpbrXxORpkzzWSAU0bLGVMcCGFbDz
# VNtaCdmR/aXzDhpjURayQkwNE8DMaejz4pZH4c36G2YLpouCoej9gSx0mD+HnfnG
# vm7n1nY9Bhtc7/GyOWqUJroSbyFlXvrExUK/3kBRD5DLjFvWugbPkGcDfqkfglBR
# Jz3yUTtIn7AOKlEJEJ0kDL0L+hvn0kVaq+FgF9WuiNI904lRuXHloyg33ZQbB1uL
# e2kaYjOYOjbVOyiBo1LN83ba/MQIyyt8a8WrsjDsg/5bhcZ0qLeQUi3ZaioNLZW6
# sn2hijtNkCYg4O0azyAlDYqpOJdpg5KzFdL6LUwvOpUi0qru8TFBp2phM2ojmNj+
# LtGwEGrmSSc0aDXszK0mxH0WEYIYiqwg/9zPO7cnpaVRxtVRxMIdjialDvG5uZYJ
# KpGhghd3MIIXcwYKKwYBBAGCNwMDATGCF2MwghdfBgkqhkiG9w0BBwKgghdQMIIX
# TAIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEEoGkEZzBlAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgFmmgyW5k3HwTr/5f9gYqA7vNQ6ar
# ehP9EthH3C+Vc9ECEQCmP/P74O82vAdMgJIMzgVRGA8yMDI1MTExMzIzNDM1MVqg
...
# MC8GCSqGSIb3DQEJBDEiBCCZK9GxKg/y6fWOHZXQ7oe2lCQJoPY7SF8+wB1FLZVc
# sDA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCBKoD+iLNdchMVck4+CjmdrnK7Ksz/j
# bSaaozTxRhEKMzANBgkqhkiG9w0BAQEFAASCAgC3p/eZdqJ8fiTvYuNTpA0bJrZO
# bjC1dYwjpE3HDcYR5ua0Vj2nbrUiPEJ00H1Wqr+Hy9mv7WYI1fnycdaAiDYhn3sL
# m8OWI50lyTlwTNpO3uO8XJqo6OXXGoZYUKNQ/a6bCAROsQ/myqwvG7XxUG7pEN4+
# iV1xutU5u5yIbVdS4m6pxh0guJkZoXQowcae5Z76iNXUDKD6QVfGIdxRy7JmVZhU
# Zlws8QiMCmsZQKYyK9Ktf9ew2bKMboMIz/gZ8NicA8WFV7IEUX7rkaaw/SLSZAqv
# m5lanLpigyt8h5o88EuYBTw4fSQxNtm7C1XhJ+7HeFpxfXiMc6DmWQXA6HNTwBA7
# dO3vZeSfZYRQBPxmySuTi+931c8t4KKwTRVa8Bd8D2/J1ua5KXl/kTlBm88m5FLz
# kBoEqSz52USUP2hoDDxfYcrdPxbOb9ifPbsZJ+u3QwXgPHmiEZeMIySEbP6ogp5u
# IvY88qfMjmyvW3ooX/EhGt3OUWxPqCoucMMogw5/mtjbbsaKoRXA4Y2aLppnseKO
# ZMfjeFuruUCjOgFE/CJ7u/5GUZ/2iRj6MKb6AO69crSVd/fcVryw5l8s63DSfP40
# Hs3jHOoZ2Whl/yZNLW/FpcrcxZyVWfhc7LYOStCwIZNghtO3tScl1+SvIQbFD9+n
# Y9Odaoy21yJTShGTqQ==
