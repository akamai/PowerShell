function Set-EdgeKVNamespaceDelete {
    ...
    process {
        if ($ScheduledDeleteTime -is 'String') {
            $DeleteTime = $ScheduledDeleteTime
        }
        elseif ($ScheduledDeleteTime -is 'DateTime') {
            $DeleteTime = $ScheduledDeleteTime.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
        }
        else {
            Write-Debug "ScheduledDeleteTime format = $($ScheduledDeleteTime.GetType().Name)"
            throw "ScheduledDeleteTime must be of type String or DateTime"
        }
        $Path = "/edgekv/v1/networks/$Network/namespaces/$NamespaceID/status/scheduled-delete"
        $Body = @{
            'scheduledDeleteTime' = $DeleteTime
        }
        $RequestParams = @{
            'Path'             = $Path
            'Method'           = 'PUT'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDCR4A0KqVXNJ17
# kKZX/XvjDYyLyi+BUIBCmIdPIyOma6CCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQg/BNRsfMb/lXizGfp9RmL3/vXRvECjPWmeiX8RWjm+PMw
# DQYJKoZIhvcNAQEBBQAEggIAfoRgrW0ybH7edus8cG4KHkMpVLePp647gs3is8E2
# Nsj/o1ZsAeuTSCOirws2TQgOxkDD76o6II4hOutI767nHCBnbcbAbu5mzIfPxOHq
# n43xPcT/TQaCI3CYKMVvxHlXrDAtLMEHrzhQU1QQGvq3rx01yKi3lV0so3etwB9S
# oECXE25m/TUZb6j9XIrAU2H3YcMS1X9KiSZk4d7NCAsyQlDbH7CW1PNenQRoZZCO
# FslwH+VljUUYx93y1UuRHdWWaOO8+BhdHdD1YIZqGfbnA4CFfs3djJPPCTsOg37X
# P4nd7Ra2/3w/7Mmg/CkIVCFemUOr5BXXkAFQW9fykD42UezZbFClJ9OELTiq1GlU
# G/ecrE9McAkGCdA7/xl8caOGabmW/Y+TXwlkNbOX9Jz+GTT1Uvho88OLtZfAdps2
# l6M4gB/7h3Vu1dFbXZy8eb2qOcqHSYZY64og1LtEXGt5P1zXRk8HFWN9S9bdXuSE
# UNRamv8wlSPdmsTLDdIE26lIfuxjKsBH33x7++yQl52IVgotqkwlTExQ6gRHY6ru
# 9Zv25pMj/suCQgdwH9cG4ebdedTOGit+hzSMGk9pWfvRTYk/bsU547Ek/NHGJH+F
# Js0FLeYEVWHdHDMRwfI9S/GuxC4A1XloiR1kN1XAoB7VDJwht6GSZs13Meim66pw
# x4Chghd2MIIXcgYKKwYBBAGCNwMDATGCF2IwghdeBgkqhkiG9w0BBwKgghdPMIIX
# SwIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3DQEJEAEEoGgEZjBkAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgtIfwcw6Xt4Oh73Es1Q/0rb9uB8LM
# MoRxVHPUYDKtI7ACEFIEeZx5BrtGvR/ciR0gAbsYDzIwMjUxMTEzMjI1NjQ5WqCC
...
# 9w0BCQMxDQYLKoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI1MTExMzIyNTY0
# OVowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQU3WIwrIYKLTBr2jixaHlSMAf7QX4w
# LwYJKoZIhvcNAQkEMSIEIK9WTrbgSXFdTdHw68EebsR1BUAtJ6VsaXyM1gkiKlRF
# MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEIEqgP6Is11yExVyTj4KOZ2ucrsqzP+Nt
# JpqjNPFGEQozMA0GCSqGSIb3DQEBAQUABIICAAL93W9dn+Hlcj6TiiBIyISdE7YC
# Lw/od8ISagkxiUyTJGhBCwN1tfGqvuDzTAh/RsgBO5tlfbZwBTTQVfNd9VnAVL+T
# 2Gzhb5wbTRaFaFWvo1zC7xORbvkaBYSG5g3FK7cEb1S+MBXWQ3xxPtRTNIFAOFrm
# T9yVfAOZbpeCKn/ekJsCbW+3OWYKEldULF+K2naM5hrgoi2NQIbcqrIig9sSqygG
# CJzvoqhNIGJINkGum64X+GXgTcRFrWZN4KbpIUfSGMpexBvHslU6tsFYdqpU8c0Q
# EH5eQat/YK+fz6Yb2nYZ4gH0QIj/wQ8gmVa1uI23YbQ/C7GypgFqHHWMovm57DN6
# rXGXVahUJe3E2S1RwEM7Xn9eTPgch10WpgSDFLe2FdxsqFYI4hz54J30SenW6ZLc
# HNJ69H9HPIv6AsaUv3VO+hx3t30lQYxxv8p3B7L+WB2ltD0GLjvSNo3y336lmF0p
# DnQo3Hh1aQffiplHLJLN3Vis/9Wj5jb0Qcjb4T8w2D+BXWFKwejvtpPZwxbk2Gfd
# koO+vh/WZALT/HWgsd4XMsaXlUE6xvc5LacCcyBT4kScAJW8/AMcG0EpXZDDAbfS
# vYqVdLqLXghOYUQUUeMZtif1z4NyiL/D0zDVzkqDXY+1a0R+gK+iDT1mUgF4DPQI
# hzfIqXLG1DLrJOdm
