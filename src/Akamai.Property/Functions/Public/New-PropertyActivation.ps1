function New-PropertyActivation {
    ...
    Param(
        ...
        [Parameter(ParameterSetName = 'name-postbody', Position = 0, Mandatory)]
        [string]
        $PropertyName,

        [Parameter(ParameterSetName = 'id-attributes', Mandatory)]
        [Parameter(ParameterSetName = 'id-postbody', Mandatory)]
        [string]
        $PropertyID,
        ...
        [Parameter(ParameterSetName = 'name-postbody', Mandatory, ValueFromPipeline)]
        [Parameter(ParameterSetName = 'id-postbody', Mandatory, ValueFromPipeline)]
        $Body,
    ...
    process {
        ...
        try {
            $Result = Invoke-AkamaiRestMethod -Method POST -Path $Path -QueryParameters $QueryParameters -Body $Body -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey
            if ($Result.activationLink -Match '\/activations\/([^\?]+)') {
                $Result | Add-Member -NotePropertyName 'activationId' -NotePropertyValue $matches[1]
            }
            return $Result
        }
        catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDM4ETymLNIkhfH
# kzCvs6VJtN7NVjouPUAWzUUSFPJhRKCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgqD/nrwLOh+Fz0mEuIa4ee5fGqN4QyBJWeKwelGin3www
# DQYJKoZIhvcNAQEBBQAEggIAcsS+JrJbjQMprBqUK1HhpbNzMy/+JXV5fPBNxXrS
# u7J7KG0rGs+Fpd7CLiumOuuGlE82D/zGu2lCZh/1rcEIK3fQw8ghgwoq8rECx5u+
# wPmvrINwdjOLB7ZNzmE627Vf/MY+UZ/O7iZwvK+CYQJGcCS358V1IthQJ9hKxwOC
# aOWI8s9kyCzA2JQ1bsVGw3XgriceCgHl+6rKQ88q6ITnUqb8YqajCKdMuNrb9QF8
# 4r0zBBn3jwY13cOUiZzloCuu5B8IKRUvsUbpMWOVKkV5CvcQ37KV3RCE4MG4Fyrw
# iR3IBIfNTSK596QWU9Cd1R83slpwKG/Ut3lvr841f3HdQrbwOSgJjz8Sqbww/sNi
# GlUg9dH3r81T+IKxROdwv+1hwYe4x4kxX40tDteCXhWk7ccRfh4O/e/rooDkyMI3
# 3EoaY0iew+0GFhpMEA4XU7SozcpPO+oBimqN+NnfD9noJIlt+GNuVGg3ALm8U22n
# ei+oaqhZ7WOHGXHLWDZjxYvOoKg80QrC7IrB5jhse9sSqWOX7kaKhQJWt/bop3p8
# Mul6wLv+4T0zuIKOFEeEG/3VqrEkwxtyBu3Ovcdwc3kaMzBLVWC1RRCKcp5Z0Gyp
# rboSzYqd1Sa5jORFuHsHhBVPCdmM5+byc0YNVtRT+6JGqYS7qz+dxa4QNesDtzuH
# oyqhghdAMIIXPAYKKwYBBAGCNwMDATGCFywwghcoBgkqhkiG9w0BBwKgghcZMIIX
# FQIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEEoGkEZzBlAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgQKP9cMH/9kG0rEdJyrjvY5n6yV2Z
# fBaf1KS9cTVUjdACEQDFjfc/gkykEX4gNgZobZf6GA8yMDI0MDgxNTE3MzAzMFqg
...
# DQYLKoZIhvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI0MDgxNTE3MzAzMFowKwYL
# KoZIhvcNAQkQAgwxHDAaMBgwFgQUZvArMsLCyQ+CXc6qisnGTxmcz0AwLwYJKoZI
# hvcNAQkEMSIEIAyb8BSpmI3SiRG4djY5A+WgpHaiiIUASL2xs3TIiHYDMDcGCyqG
# SIb3DQEJEAIvMSgwJjAkMCIEINL25G3tdCLM0dRAV2hBNm+CitpVmq4zFq9NGprU
# DHgoMA0GCSqGSIb3DQEBAQUABIICAC3BygPyQY4D7CQOmYVSy96gKivbCEhGyMta
# 1nNjCb3dw1LKyMnCjJlXxqIvRCmAWOa7AkYAQOl5V7kfAM+V4QZki59ZaKz5ro7i
# YrfuPm4HmZiuxJUf54KWe6+ghsZKzjOaQHm/7bxagKXtfjW1gNLaCPEBVzc1YPTX
# wOwE2+7gUcr9lRZ41z9A+nYUCw4IMsGJqHYgY+PloDMUjb9vrmLuqbt7QpG6G9DV
# WFg4Y1rPrXk7zeXbl56qXO1Lgdu4ZekQhxssKVwcR8+N4e2/I2QO3QASSEyMR9nm
# 0ivY1KYVTaJduydfclYiWZwpH2S1DI5i5RiG9QqDB7jC2Bv7QTSA8f4bxFOUUquw
# 3Ed9jLPxSUHYtoZq2Gx0r1YKVPMNfN2+IV7wRMr7UIWFyCBecBofEYqKZiA/C+D1
# AUyGG985iSyr/iaGQ8PPfwILywQfwv/OlhJlxqhtOtD8qYOffYEHvD4Gpq18rV45
# URVyfbBC4d+SwBSw3e0jMgI6MVJ5XnR82xNBBEarII5RKEGK4thNSper2agWYLfj
# w2zhxl96QBaw9Clo8t06ZgNtL+gL3/sc3t+PzcV/EKRh8nVh25bT+z6hGKEaZYnl
# Ck/3faIFzKp/K/KKefiWcLV6/nlWDi4c1Y81V3eZu70AA/2Lni3tWrg4fxEXiW8+
# IcbVa2FP
