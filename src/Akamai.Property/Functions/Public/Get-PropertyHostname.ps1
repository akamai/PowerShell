function Get-PropertyHostname {
    ...
    Param(
        [Parameter(ParameterSetName = 'single-name', Position = 0, Mandatory)]
        [string]
        $PropertyName,

        [Parameter(ParameterSetName = 'single-id', Mandatory)]
        [string]
        $PropertyID,

        [Parameter(ParameterSetName = 'single-name', Position = 1, Mandatory)]
        [Parameter(ParameterSetName = 'single-id', Position = 1, Mandatory)]
        [string]
        $PropertyVersion,

        [Parameter(ParameterSetName = 'single-name')]
        [Parameter(ParameterSetName = 'single-id')]
        [switch]
        $ValidateHostnames,
    ...
    if ($PSCmdlet.ParameterSetName.StartsWith('single')) {
    ...
    try {
        $Result = Invoke-AkamaiRestMethod -Method GET -Path $Path -QueryParameters $QueryParameters @CommonParams
        $Hostnames = $Result.hostnames.items
        if ($PSCmdlet.ParameterSetName -eq 'all') {
            if ($null -eq $PSBoundParameters.Limit -and $null -eq $PSBoundParameters.Offset) {
                $Offset = $Limit = $Result.hostnames.items.count
                while ($Hostnames.count -lt $Result.hostnames.totalItems) {
                    $Hostnames += Get-PropertyHostname -Limit $Limit -Offset $Offset @CommonParams
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBWlVKW2R8o+6wr
# VGl4XhM35Ye1W3xLcFEQGC4cOa1EWaCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQghv1sd1klDeglFc6uYY5kesjjq1NCJB9j3FDK8zWm3u0w
# DQYJKoZIhvcNAQEBBQAEggIAagPYhu0lC88ZGy3WVoAVOhZKHav5OKmXmYRPZdQw
# miKTIYenFSZmw3PVqZUXbp+hEI/lt0bbUdgm2/2kH4IrTSKBa3rpYuSWUvRB8XkZ
# uuDtzjqQ09oV2TFKqyUz/YF+ui53ikbFaMmGY2bkBmNHcXjG2TsQjBUMHINtB8+l
# 0Y6K81T9mAVZrHTZLGc8qcZBcogNcs2EiduEyTgEIM7hNJXLWT+XBlq1TGOogy4i
# OveJjFsb6+cYmHwNAMXzCbpB7Uq6i49FPNyU3oyvxAugIHGd3QZ+4JfyrKhDBe9v
# ejujvHsG2XYBA47Pd94uIp6u9m3iVf9X2cRYT9R1ZGIV6oLLlFUQOU0sZLM4txA9
# mOTGa14x+zKgHakZSjMq3nQgimJNLEpIUZcUjU9799p3saiUQfeGxbDOQ6CYN4he
# wsspC7/fJcxAPWhFYiGLpVeJSQu8zwI2pTZtRxZg/jgHX2vSy7wtxrSdHyVri+r4
# IkYGmGFtUAroiWRgMPaLKSgAcbjKvMEvKE983A0hyrkC+TtHntH3akuJNvMuvP6S
# MTRzssbG/kWAmHDsR30jfSFDcAQS6ZgmOz5H3PXBD5/PZNe5FbjNy+NWqk3FX/tB
# RBubx5c6ztXkNIlOZKEkYSklrEkLdjSXM7GbzptIRKFBTUUgU6b2BXNtAmnxJHKB
# qrWhghc/MIIXOwYKKwYBBAGCNwMDATGCFyswghcnBgkqhkiG9w0BBwKgghcYMIIX
# FAIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3DQEJEAEEoGgEZjBkAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgLcedb3Er+DSCG01gSr2P9eBwp6KE
# MjFnoLobdji4qZ0CEFGQuXi0zWIZraoRtTlUSHIYDzIwMjQwODE1MTcyODU4WqCC
...
# BgsqhkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjQwODE1MTcyODU4WjArBgsq
# hkiG9w0BCRACDDEcMBowGDAWBBRm8CsywsLJD4JdzqqKycZPGZzPQDAvBgkqhkiG
# 9w0BCQQxIgQgYMWB8N2Z57nc7k+SlIKUyZrBDsoNR1hw3GiUKVKHGRQwNwYLKoZI
# hvcNAQkQAi8xKDAmMCQwIgQg0vbkbe10IszR1EBXaEE2b4KK2lWarjMWr00amtQM
# eCgwDQYJKoZIhvcNAQEBBQAEggIAZRqa5rj+kA/1npY+KpWlyKg4+V9WhIHQIv0b
# JNH1URfN2S0cYIL0EtSjDfiG2zhuKSwsPmadvU0lj2CZWFn7VXbok4qtX6vDRLdF
# MNmhq6Vh29Q/mlyXXoIPyD9GqOpvqP5qhAo5X0pRJENUrEbexjsEiykN7cVyHqbo
# ffp98VhHb+ptOQMA6IOncbLVz08ZClxBQ2iZvrU0cNZbff45HtrDVuFDtuS5ZOIv
# ayMnzV+h4k8/k4YOQuW+O7PD9TpJvYgpoBPjUhLTRbfV97jGTQ9jK+rA6ED2rKzN
# BOgKvNIDzu4bePDjKjiGjcwnrZ6nEJWjJEiHnI6frE37nqVzyqHaOG0MhCpjb4w6
# ODkfqKtl7Hz5D8E59U7iywx7wl3cOLsVlojltqw5XzECHNscBYxgyZOxMzpnQAYa
# MaanbVT0dn/H41N+jHga3kbzCLs6B3Y/0n5qIZ19Z1xOghqxtPmFZ6PrrqaL9p92
# HEVHyL6zuErRnt1EVow/hl4ozRlrP2p/iPI4vNcoNs5/4fQ5FGsulpDrTHocjhYZ
# m/wtW08e6BNkn7TBeR2cq2x9x/Ygl07so3oWgUPYYxtfHdTY6Zad16D29yL/hIvN
# E9bmPCbFUzkhyLOdhKvVmpCi4J2Ob1fs0BgUXz6QyLTKn3UHtBNGvCzuOP9+w/+A
# VGD0l2w=
