function Expand-AppSecConfigDetails {
    ...
    if ($ConfigName -ne '') {
        $Config = Get-AppSecConfiguration -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey | Where-Object { $_.name -eq $ConfigName }
        if ($Config) {
            $ConfigID = $Config.id
            Write-Debug "ConfigID = $ConfigID"
        }
        else {
    ...
    if ($VersionNumber.ToLower() -eq 'latest') {
        $VersionNumber = (Get-AppSecConfigurationVersion -ConfigID $ConfigID -PageSize 1 -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey).version
        Write-Debug "VersionNumber = $VersionNumber"
    }
    if ($PolicyName -ne '') {
        $PolicyID = (Get-AppSecPolicy -ConfigID $ConfigID -VersionNumber $VersionNumber -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey | Where-Object { $_.policyName -eq $PolicyName }).policyId
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDM+mWKLPXB153P
# s93IJKVszRFoAjdDV/8mmt5zik+mzaCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgI2ny6YXB9fcDPJPr08ZMV+qJNBt1hANOfdxw0QVcVOsw
# DQYJKoZIhvcNAQEBBQAEggIAeovHBgJrvPqB06QNZkQCd1gbufhKudCcdUxnq6Fg
# eeJuaVUybyi23EpG1WG0Y76uDpUWT7XGHHUusjDhuvoVo03tgLk60aIdxSz2M03M
# sA06qeq3aowgbH1tEzvfIaeEbgzFJ/jUpNU2kAmeZfxNt0EZEd3vGX/sQQXV3Bbz
# rKwNGTpGb/YZlTC403pLfA0XHHy5Wp+dJ912zpzaj0WW193hP+zgiJwk6zrVse7q
# dYmhrEQyma5zxKOcgadJQbdnIvbG+HYCJaqOXFfhn6RcXareNOsggnvHFdgmWbWC
# K5b5nIlqpSPWpbgmEOMLKk/AlOcqx51bT4qXmnJB41OW13Zi6cQrmmHknlTZvN1y
# UJMtoWCPNfZnhv/p/48MVht7NjhBv9Vj8LSg7i3wSdwj+0GCgzQ3SiTCm2Nbg8r/
# zc0JZKw9mU1gsJoSbZtSLPvaJVkqmpoD0uhHpbwBD9Vi2DOwO+UuzuOyvSI6GKJw
# FHefFmQ4wJR3dpDgEfgXnmpN+xlkk5YP/+aGF4BrH4Dxd9r81vw5/C2/SFMny76J
# 4Z3BBK6yzdjcm7Law/PSelJUO714+YObv0CotNcFuLbdk8heetiAjy2IpZRppLZf
# pc56yWrpg+2FOBJy53ndDKAxOL/BgDxDuLUyCKGb2xU+v5gX0n+PSAUe6sOJQBt2
# dAqhghc/MIIXOwYKKwYBBAGCNwMDATGCFyswghcnBgkqhkiG9w0BBwKgghcYMIIX
# FAIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3DQEJEAEEoGgEZjBkAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgQGLsqN9t2PIgbkdJouAjqo6sBcs9
# vsTwlnY22WjQvj0CEDGItdt7wLR0aKr1ymXTDOQYDzIwMjQwNDExMTQ1NzEwWqCC
...
# BgsqhkiG9w0BCRABBDAcBgkqhkiG9w0BCQUxDxcNMjQwNDExMTQ1NzEwWjArBgsq
# hkiG9w0BCRACDDEcMBowGDAWBBRm8CsywsLJD4JdzqqKycZPGZzPQDAvBgkqhkiG
# 9w0BCQQxIgQgfvIRU/TMARnvQBVb/4lvw6NuqAKuElu/x0BEQia8iFkwNwYLKoZI
# hvcNAQkQAi8xKDAmMCQwIgQg0vbkbe10IszR1EBXaEE2b4KK2lWarjMWr00amtQM
# eCgwDQYJKoZIhvcNAQEBBQAEggIAEPPr22tDrvYQHEPezFwKaisJgEMXQ7eE6be3
# qpwx8PkjCdERkshbT0Ru3Qc+ERM0udL+qEXr5Xdae/q+OJQkrVUzaqb9YPn8ar+C
# 6bGyKE/qWcY96k2amaY2DprilD7u5p8we3v1+IWT5fg7foNWoNlMdGqMb5KieRyM
# l7orR0rfthvfl1WjWaC2ug6+PXs/EsFwRsxYBiZmycM/44n6VbBsHhfgabirFBW/
# FJKKlULcffHaQWCauNiiN8nlMK+YqRO6jHcgmphMPlo6tkxlzyqzn30/6uVy+XEB
# PUzW0sy7ppowMS8EXfDDtXsIznMy3FGLYPUJeUAtHS3Vo8i1Iacd1oCj9lf911lJ
# e36pBp/UmhGvPoHS0BoUX87fM9aOs/zkHZ1+TQY8FxPbqFjFdNCOaA6t1arCGLP7
# 9/PWMuIRUJH2F3rwWPOohbC5hvpKppZTExnzSz1i3WL7/vLbfvXsfQJV4BNdUf0H
# 2EvZHC+rrJyi0E3vF+1Eavjbvactjkp6cb1A/thd8eapOo9mN6gcIZaGSBgRT0Lu
# NrWRYbEAWUFeoGLkUyiWCjl207E8GTFqaqRknEqcUr3xVH4ZzTspc+utGU6C9Z2S
# Wu0ePTUTYQTeR4ClUNBj4uk/IiVhSscL3IwWFKUwgpT9uJpRYLd9v2APiWSgSnN7
# aVt5tEk=
