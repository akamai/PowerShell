function Get-AppSecPolicyEvaluationRule {
    ...
    process {
        ...
        if ($RuleID) {
            ...
            $Path = "/appsec/v1/configs/$ConfigID/versions/$VersionNumber/security-policies/$PolicyID/eval-rules"
        }
        $RequestParams = @{
            'Path'             = $Path
            'Method'           = 'GET'
        ...
        if ($RuleID) {
            ...
            return $Response.Body.evalRuleActions
        }
    }
}
