function Set-EdnsRecordSet {
    [CmdletBinding(DefaultParameterSetName = 'single-attributes')]
    param (
        [Parameter(Mandatory)]
        [string]
        $Zone,

        [Parameter(ParameterSetName = 'single-attributes', Mandatory)]
        [Parameter(ParameterSetName = 'single-postbody', Mandatory)]
        [string]
        $Name,
        ...
        [Parameter(ParameterSetName = 'single-attributes', Mandatory)]
        # [Parameter(ParameterSetName = 'multi-attributes', Mandatory)]
        [string]
        $RecordName,
        ...
        [Parameter(ParameterSetName = 'single-postbody', Mandatory, ValueFromPipeline)]
        # [Parameter(ParameterSetName = 'multi-postbody', Mandatory, ValueFromPipeline)]
        $Body,
    ...
    process {
        ...
        # if ($PSCmdlet.ParameterSetName -eq 'multi-attributes' -or $PSCmdlet.ParameterSetName -eq 'multi-postbody') {
        #     $Path = "/config-dns/v2/zones/$Zone/recordsets"
        # }
        

        # if ($PSCmdlet.ParameterSetName -eq 'single-attributes' -or $PSCmdlet.ParameterSetName -eq 'multi-attributes') {
        if ($PSCmdlet.ParameterSetName -eq 'single-attributes') {
            if ($RData -is [string]) { 
                $RData = $RData.Split(",")
            }
        ...
        if ($PSCmdlet.ParameterSetName -eq 'single-postbody') {
            if ($Body.RData -is [string]) {
                $Body.RData = $Body.RData.Split(",")
            }
            if ($Body.Type.ToLower() -eq 'txt') {
                for ($i = 0; $i -lt $Body.RData.count; $i++) {
                    if ($Body.RData[$i] -notmatch '^".*"$') {
                        $Body.RData[$i] = "`"$($Body.RData[$i])`""
                    }
                }
            } 
        }
        ...
        try {
            $Result = Invoke-AkamaiRestMethod @RestMethodParameters
            return $Result
        }
        catch {
            throw $_
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB5zTLlSlLeX/2V
# CTW0yn2UnA1KfncS2aI4tW7NQrZM8KCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgasfV3M1NGMbo2B/3PGkvRSL7kKpL5+9W9iZDEFmtKjQw
# DQYJKoZIhvcNAQEBBQAEggIAGKYwyJ7pDT74RgeEtCxUA9EvRd8u1SmHaBpxPpcU
# vH6dYc1zrJ0Q6oSvrwCGH7lWJn2heWzVXnfJ2Kkn+df5T62cMv3l4YD0AhCWdXVF
# 5AmpbPLX4ip/859T55oe3YszmqfvHNFARbHrbwre5ivlJ/BBoU9Q2WXXkNY7nT65
# IxJZbWP7jebzyfd5k59xFjskkMRp7lQv7qML9k6/kftcir3MU1/8qqHQZsxme+kq
# e9Zmhu8R1KTtHfzW+xoPfQ9NfCPelpQZ/QR/+wPOXoba3/8wv8xXweGvmqGUucsT
# NfLvKoxNi+BjO14XfAGB5SeCC118ctIZHN5E4qpiSLxKvX9aehxyZqxFSKJ0L9Mp
# 0ZgvSwE+47LFCTsYLo+7KF+nM+M02PsvX01pX4dqQYHc6LurSHhvTCF88ZAszbne
