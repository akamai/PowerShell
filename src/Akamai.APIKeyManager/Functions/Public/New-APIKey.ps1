function New-APIKey {
    [CmdletBinding(DefaultParameterSetName = 'attributes-count')]
    Param(
        [Parameter(Mandatory, ParameterSetName = 'attributes-values')]
        [Parameter(Mandatory, ParameterSetName = 'attributes-count')]
        [int]
        $CollectionID,

        [Parameter(ParameterSetName = 'attributes-values', Mandatory)]
        [string]
        $KeyValues,
        
        [Parameter(ParameterSetName = 'attributes-count', Mandatory)]
        [int]
        $Count,

        [Parameter(ParameterSetName = 'attributes-values')]
        [Parameter(ParameterSetName = 'attributes-count')]
        [string]
        $KeyDescription,
    ...
    process {
        if ($PSCmdlet.ParameterSetName.StartsWith('attributes')) {
            ...
            if ($KeyValues) {
                $Body['keyValues'] = ($KeyValues -Split ',')
            }
            elseif ($Count) {
                $Body['count'] = $Count
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAWrzmXE7i7z5Oj
# EzDRDCmUMb8AjuSaVYbFGsgD/k0SY6CCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgDHH2vmIXxj2ESAhUQ7CQElSZUBOgLd2THvhhAMG6Wx4w
# DQYJKoZIhvcNAQEBBQAEggIAr5Z1SiLZRq+q7TZdKQrMT1NIgVSDZbmwGZCO3STo
# UTiiux3JPV3Mj/W36Ow9WjijsSEzr60ZU9dByl57nrimPSY+ztlXTPkv17BW5jsN
# xuxfzKHn/TwPf5202veSNCNJR32R9TByk386ryNviJ3k92HSZFflkBKSw89givfF
# pCKXzzzcxWu32C0ZICZqG0awVE3o+8Fyt4oJkLPBDjg2sLA1Va7+qCd+pIWISvOQ
# PKNLrDUBL8F3GWuT0x6qvawvcQyygbRua97Xri3SbMHk+Z/XwusnVnalyQ3sSoI3
# B3UFdmvAMNyxF7nh+4R4R+RYNTDf2nwhY7f1CeRF4zPfVU3NtdR/LWRXBDlI2lPe
# DCeqmG7EVCHnuWAhHzZVVFtuiZRWPoRefbTDlBOrIlPPyBWhAtZ1vayBAWXhKyp8
# IUv4uFcIlXHzMksDkqPeT7OdboO8hOi0Fc+xhYQVFva4Ge4nAIqczHKbc8gxMdHC
# btT/+Y8dy1txu4pNM2uFGHbQp5XPE/4b1zO05ForD3QgfRUdK+wXzvxsvbSsbdRe
# M/vqHl701XvrAS7os2HlMs7Ztcnt2rOCezFSJSTSJlvPRkXRPdtv+y/Yis/RT6bT
# UiKeNBP+1xEZqGNdnOae2S995srmqlIJLMoWB46ElfTUFfFQyMKpvNETaFe9T5y4
# w0qhghc6MIIXNgYKKwYBBAGCNwMDATGCFyYwghciBgkqhkiG9w0BBwKgghcTMIIX
# DwIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEEoGkEZzBlAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgDpl4P4Zz6Tlb7v/qptzHfA0QlUEe
# lvifQUYF4X39upsCEQDhrF1wnZNba83IGAsn6MEEGA8yMDI1MDQwMjE4MDUzN1qg
...
# hvcNAQkQAQQwHAYJKoZIhvcNAQkFMQ8XDTI1MDQwMjE4MDUzN1owKwYLKoZIhvcN
# AQkQAgwxHDAaMBgwFgQU29OF7mLb0j575PZxSFCHJNWGW0UwLwYJKoZIhvcNAQkE
# MSIEIKWYKt2/rDBXxwGREPGagLtzI0+1i7EzCsGQwFtyVex9MDcGCyqGSIb3DQEJ
# EAIvMSgwJjAkMCIEIHZ2n6jyYy8fQws6IzCu1lZ1/tdz2wXWZbkFk5hDj5rbMA0G
# CSqGSIb3DQEBAQUABIICAEgXvQQ1sMFTh1BJTBwZqUoa1MYiR95Vh0vleI8KScLS
# n5f+6xoU5E8gMXs9RcPIC441FEsu3DGAuZ9t55TLm25XqG2ER45UWuHsQ484rqT5
# vkuTTBu7uCGFK2LUPE82FfvVOXZtC64ValjYdnXgAM9MuzEYjIWQmd2aTznmboOr
# nEndbAgmrvK20UpK5uSwbI0SKutAnQeFu9GwN7G/4Ec0n4ll/ntMo634oueqiAUj
# O6KEekeS3bSxnUivMYCrXzxmKW58+YcJkL6P5clA3b5sl/dyRMcZiq6CwrWidAQw
# T1ArO3noETGBvvr2ni9u1rp6PGRZ3CgHkAbqC0rYezq/OqIAO88o0FZubuZSOmLr
# yWkAgbXSxnVh4I+DF9ZBpoDg2VHG1dqBBvMRcW2tKrmM2FZgw2Fp4YuwoshWOMFD
# 18TIGYiPxm6wv9SiyqMsoOrYD4qg2bOwQWtGoSTYm9QB/flcXnK+g6TxqueFvQC0
# A2Zr66gZIrqrEcEH/rXFRlnxZgM1SLmA13FonnIII91NA2Cx+CS+O3SOVypUCiPv
# 9WuDzKD0peRSgnnGaEwR9xM9RvIShFFcX9/XlIQv4Gg53CDTdeieNVdQcfHrZS4c
# MN10d22VmycLmZrxhbprXNPv4OB1QvcyJVGRPkV/jxSecKE93kCDgVzswOVBnBd4
