function New-EdgeDiagnosticsMetadataTrace {
    ...
    process {
        ...
        if ($PSCmdlet.ParameterSetName.StartsWith('attributes')) {
            ...
            if ($HTTPMethod -ne '') { $Body['httpMethod'] = $HTTPMethod }
            if ($EdgeIP -ne '') { $Body['edgeIp'] = $EdgeIP }
            if ($MDTLocationID -ne '') { $Body['mdtLocationId'] = $MDTLocationID }
            if ($RequestHeaders -ne '') {
                $Body['requestHeaders'] = ($RequestHeaders -split ',')
            }
            if ($SensitiveRequestHeaderKeys -ne '') {
                $Body['sensitiveRequestHeaderKeys'] = ($SensitiveRequestHeaderKeys -split ',')
            }
        }
        $RequestParams = @{
            'Path'             = $Path
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDdZ8hE/kkACgbK
# J7VzEQNweCt017p3/4/4dLzunINUyaCCDo4wggawMIIEmKADAgECAhAIrUCyYNKc
...
# BgkqhkiG9w0BCQQxIgQgrzVV4RDfdH8cSzBhjp4oSCmEQ1SpBxZlqzZOtl5VQTEw
# DQYJKoZIhvcNAQEBBQAEggIARGk52qQsZNR/NRvbCCbXgOwq0teD6iR2ruM6QrGw
# JCt984hKHMZgT55ZCcWAF3wuST9pA1DwunYZMK6gKJ/pTYdVf5oYl9dKIv78UBG8
# K20KtnAhzNhc6kyUZMJU+GrWOD5imdlNXBEd1wAn1DZVU9pWAivkt19woGQsJwhc
# ydf4xj6FkfrTjlE1OpuUUdx9aXhco4CY3V9OiDTJ7xX0QzvgJmzz5/hUG0OG1Pja
# wMz608cMQVvyeyRr5CRCPIBJs3FQnMP8OOVaWP8qqHIsgtk0ngzs6d+P9rci/0VA
# NTgXsZaFFzo+ytVckXG04D2gq+9PLSRetR1scYJKPh2ZTLUeNXkh6IayShIzp3+K
# 2EoGOvdB96wsUGMwnuWVZKOWuz1tb1StOzn/Xrpr99TvqiFcdA4QnLgOHfLba3Zw
# tzC2JjXhsIAlKNJ15SM5kSzdnOM3JQrR2Qhqg7/jHs2oMFHvoEFDkb7K9nzPFSZ/
# sLiSsAIydnLK6Sf6v8AWN8PcFbo+22xgdrw/ERacNbcvK53aMTtKvpN3ZZXNq4Xo
# 36JS9tozvgdYgqz9RZ877v0Yv6ZUOQamewICSvoa32dRK1kJkm063LmIs6QHFNsh
# TfKkZg+qesnyVttVXuo9doPTwU22uJCNt9vK+MGSjLyFk8+9SVxqYaA04jPONXuK
# S9ehghd3MIIXcwYKKwYBBAGCNwMDATGCF2MwghdfBgkqhkiG9w0BBwKgghdQMIIX
# TAIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEEoGkEZzBlAgEBBglg
# hkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgeR0acR5l8m2B0zKpDS3F0+YVlZ00
# mrukg6JAkTp0E5UCEQCd+2wlUSlgRbSo9sZQEWhKGA8yMDI1MTExMzIzMTMyMFqg
...
# hvcNAQkDMQ0GCyqGSIb3DQEJEAEEMBwGCSqGSIb3DQEJBTEPFw0yNTExMTMyMzEz
# MjBaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFN1iMKyGCi0wa9o4sWh5UjAH+0F+
# MC8GCSqGSIb3DQEJBDEiBCDW7Sb8vAldEgrv6X3Eq95ObuezHFK4oCMVyQxhxp4Z
# 0DA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCBKoD+iLNdchMVck4+CjmdrnK7Ksz/j
# bSaaozTxRhEKMzANBgkqhkiG9w0BAQEFAASCAgAkarqPuQqZ12e18M+ercMEqOlr
# codtN09UUDgLue5nNbHVVwn/1qv+0xvzPDGuBw4kmlzRuoH5S8+fHCeKDzjSYHmS
# d/RdHB7ToLbmzFjI6gUBH0y+UpxLOE7l+OUDLqmogzb6XqhTxmcqwvkBKmSdvhCJ
# GeemquDAKlTn7jJkhLEVhvkXAqKqimyKe59FyPcz057kkU3QopBVBBRqewOshd/w
# 488BJ1bNepqEf+keh0LbExwizSXv8BQg6YsTIhQH2ZU2D9/iBx6I9h4Dzg8d8S51
# uERxqgdosqPUy3UJn/PZRtkokAp5FT6QUt+Zo+xjn9gA5f1TsMAga+CGeh6FSK7J
# ybXuAyjxY/CV8e0ElcNiDLbobavikti7mf/x10fqRw7bSzjbdm3kOlMyGUNJkBZn
# 2E6XGFAqzWwZSnD/Ic++prPuuU8d5lRJDFVK5TXIkm+rVB1tcATLi/7VX8ZiI5UH
# 5mNf5UYmo8sBG80jbvOjFObGHCndrtOg2X+qliU8fooi1bvVZhRifx/IEH+01KN2
# VXN6mIyUwE8BQsrawid5WB0FEyiTHF94b0b7GsxBHX4nMmusxmM7UBnsYmztJNFp
# VuXIo1fPcHzFHscTatCyyscFbq+gBjdWk8NbtKmFjwM3PiMSofWK3C9LHquinIla
# 5zD59hjt8CI7aft37A==
