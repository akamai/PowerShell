...
Describe 'Akamai.SIEM Tests' {
    ...
    BeforeAll {
        ...
        # Load modules
        $TestModules = 'Akamai.Common', 'Akamai.SIEM'
    ...
    Context 'Format-SIEMEvent' {
        BeforeAll {
            . $PSScriptRoot/../src/Akamai.SIEM/Functions/Private/Format-SIEMEvent.ps1
            . $PSScriptRoot/../src/Akamai.SIEM/Functions/Private/ConvertFrom-Base64.ps1
        }
        ...
        AfterAll {
            Remove-Item -Path Function:/Format-SIEMEvent -Force
            Remove-Item -Path Function:/ConvertFrom-Base64 -Force
        }
    }
    ...
    Context 'Get-SIEMData' {
        ...
        It 'succeeds using from and to' {
            $TestParams = @{
                'ConfigID' = 123456
                'From'     = 1488816442
                'To'       = 1488816784
            }
            $PD.GetSIEMDataFromTo = Get-SIEMData @TestParams
            $PD.GetSIEMDataFromTo.ResponseContext.attackData | Should -Not -BeNullOrEmpty
        }
        It 'succeeds using from only' {
            $TestParams = @{
                'ConfigID' = 123456
                'From'     = 1488816442
            }
        ...
        It 'succeeds using offset' {
            $TestParams = @{
                'ConfigID' = 123456
                'Offset'   = 'c0bc409010aa6928e57cd5a3000433b9'
            }
            $PD.GetSIEMDataOffset = Get-SIEMData @TestParams
