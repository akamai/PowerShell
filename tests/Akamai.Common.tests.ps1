...
Describe 'Safe Shared Tests' {
    ...
    ### ConvertFrom-Base64
    $Script:Bas64Decode = ConvertFrom-Base64 -EncodedString $Base64EncodedString
    it 'ConvertFrom-Base64 decodes successfully' {
        $Bas64Decode | Should -Be $ClearTextString
    }

    ### Convert-URL
    $Script:URLDecode = Convert-URL -EncodedString $URLEncodedString
    it 'Convert-URL decodes successfully' {
    ...
    ### Format-QueryString
    $Script:ParsedQuery = Format-QueryString -QueryString $Script:UnsanitisedQuery
    it 'Format-QueryString strips empty query params' {
        $ParsedQuery | Should -Be $SanitisedQuery
    }

    ### Format-Filename
    $Script:ParsedFileName = Format-Filename -Filename $Script:UnsanitisedFileName
    it 'Format-Filename encodes invalid characters' {
    ...
    ### Confirm-Auth
    $Script:AuthErrors = Confirm-Auth -Auth $Script:Auth
    it 'Confirm-Auth prints nothing' {
        $AuthErrors | Should -BeNullOrEmpty
    }
